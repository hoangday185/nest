...
//custom lại phần validate với decorators
//do phần class-validator với decorator đều trả ra property decorator nên ta có thể dùng class validator với decorator để custom
//validator
export function NumberField(
  options: Omit<ApiPropertyOptions, 'type'> & INumberFieldOptions = {}, //để có thêm phần swagger và khỏi khai báo trong controller thì ta kế từ apipropertyoptions
): PropertyDecorator {
...
export function StringField(
  ...
  const minLength = options.minLengh || 1;
...
export function PasswordField(
  ...
  const decorator = [
    StringField({ ...options, minLength: 6, maxLength: 30 }),
    IsPassword,
  ];

  if (options.nullable) {
    decorator.push(IsNullable({ each: options.each }));
  } else {
    decorator.push(NotEquals(null, { each: options.each }));
  }
...
export function PasswordFieldOptional(
  options: Omit<ApiPropertyOptions, 'type' | 'minLength' | 'required'> &
    IStringFieldOptions = {},
): PropertyDecorator {
  return applyDecorators(
    IsOptional({ each: options.each }),
...
export function EmailField(
  ...
  const decorators = [IsEmail(), StringField()];
  if (options.nullable) {
    decorators.push(IsNullable());
  } else {
    decorators.push(NotEquals(null));
